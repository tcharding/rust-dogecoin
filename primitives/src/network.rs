...
/// Error useful if pattern matching on `Network` from outside this crate (to handle the
...
impl UnknownNetworkError {
    /// Constructs a new error from `network`.
    pub fn new(network: Network) -> Self { Self(network) }
}
