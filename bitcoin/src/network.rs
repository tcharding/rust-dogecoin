...
pub trait NetworkExt {
    ...
    ///
    /// let network = Network::Bitcoin;
    /// assert_eq!(network.magic().unwrap(), Magic::from_bytes([0xF9, 0xBE, 0xB4, 0xD9]));
    /// ```
    fn magic(self) -> Result<Magic, UnknownNetworkError>;
}

impl NetworkExt for Network {
    fn from_magic(magic: Magic) -> Option<Network> { Network::try_from(magic).ok() }
    fn magic(self) -> Result<Magic, UnknownNetworkError> { Magic::try_from(self) }
}
...
mod tests {
    ...
    #[test]
    fn serialize_test() {
        assert_eq!(serialize(&Network::Bitcoin.magic().unwrap()), &[0xf9, 0xbe, 0xb4, 0xd9]);
        assert_eq!(serialize(&Network::Testnet.magic().unwrap()), &[0x0b, 0x11, 0x09, 0x07]);
        assert_eq!(serialize(&Network::Signet.magic().unwrap()), &[0x0a, 0x03, 0xcf, 0x40]);
        assert_eq!(serialize(&Network::Regtest.magic().unwrap()), &[0xfa, 0xbf, 0xb5, 0xda]);

        assert_eq!(deserialize(&[0xf9, 0xbe, 0xb4, 0xd9]).ok(), Some(Network::Bitcoin.magic().unwrap()));
        assert_eq!(deserialize(&[0x0b, 0x11, 0x09, 0x07]).ok(), Some(Network::Testnet.magic().unwrap()));
        assert_eq!(deserialize(&[0x0a, 0x03, 0xcf, 0x40]).ok(), Some(Network::Signet.magic().unwrap()));
        assert_eq!(deserialize(&[0xfa, 0xbf, 0xb5, 0xda]).ok(), Some(Network::Regtest.magic().unwrap()));
    }
