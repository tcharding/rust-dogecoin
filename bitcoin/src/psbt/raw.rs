...
pub struct Key {
    ...
    pub type_value: u64,     // Encoded as a compact size (`VarInt`).
...
impl Serialize for Key {
    fn serialize(&self) -> Vec<u8> {
        ...
        let compact = VarInt(self.type_value);
        compact.consensus_encode(&mut buf).expect("in-memory writers don't error");
    ...
    Subtype: Copy + From<u64> + Into<u64>,
{
    fn consensus_encode<W: Write + ?Sized>(&self, w: &mut W) -> Result<usize, io::Error> {
        ...
        VarInt(self.subtype.into()).consensus_encode(w)?;
    ...
    Subtype: Copy + From<u64> + Into<u64>,
{
    fn consensus_decode<R: BufRead + ?Sized>(r: &mut R) -> Result<Self, encode::Error> {
        ...
        let varint = VarInt::consensus_decode(r)?;
        let subtype = Subtype::from(varint.0);
