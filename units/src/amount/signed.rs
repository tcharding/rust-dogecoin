...
///
/// The sum of transactions over a time period is an example where it is possible practically that
/// the result is larger than `MAX_MONEY` and exceeds the total supply of Bitcoin, you just can't do
/// the sum with an [`Amount`].
///
/// We support:
///
...
impl SignedAmount {
    ...
    #[deprecated(since = "TBD", note = "use from_i32_btc instead")]
    ...
    /// per bitcoin overflows an `i64` type.
    #[deprecated(since = "TBD", note = "use from_i32_btc_const instead")]
    #[allow(clippy::missing_errors_doc)] // Deprecated, removing soon anyways.
    pub const fn from_int_btc_const(whole_bitcoin: i64) -> Result<SignedAmount, OutOfRangeError> {
    ...
    pub fn from_i32_btc<T: Into<i32>>(whole_bitcoin: T) -> Result<SignedAmount, OutOfRangeError> {
    ...
    ///
    /// If `whole_bitcoin` is greater than `21,000,000`.
    pub const fn from_i32_btc_const(whole_bitcoin: i32) -> Result<SignedAmount, OutOfRangeError> {
        let btc = whole_bitcoin as i64; // Can't call i64::from in const context.
        let sats = btc * 100_000_000; // Don't need checked multiplication: i32::MAX * 100_000_000 < i64::MAX
        Self::from_sat(sats)
    }
